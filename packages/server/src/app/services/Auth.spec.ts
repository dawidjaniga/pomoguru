import { GoogleAuthService } from './GoogleAuth'
import { OAuth2Client } from 'google-auth-library'
import { AuthService } from './Auth'
import { UserId } from '@server/types'

let verifyIdToken = jest.fn()
jest.mock('google-auth-library', () => {
  return {
    OAuth2Client: jest.fn().mockImplementation(() => {
      return {
        verifyIdToken
      }
    })
  }
})

const examplePrivateKey = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`

describe('Auth Service', () => {
  const googleClientId = 'xyz-123'

  beforeEach(() => {
    verifyIdToken = jest.fn()
  })

  it('should create service instance', () => {
    new GoogleAuthService(googleClientId)
    expect(OAuth2Client).toBeCalledWith(googleClientId)
  })

  it.only('should sign JWT Token', async () => {
    const service = new AuthService(examplePrivateKey)

    // verifyIdToken.mockImplementation(async () => {
    //   return Promise.resolve({
    //     getPayload () {
    //       return {
    //         user: 123
    //       }
    //     }
    //   })
    // })

    const jwtToken = service.createJwtToken('xyz-123' as UserId)
    const verifiedTJwtToken = service.verifyJwt(jwtToken)

    expect(verifiedTJwtToken).toHaveProperty('userId', 'xyz-123')
  })
})
